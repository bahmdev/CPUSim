...
public class PreferencesController implements Initializable {
    ...
    @FXML ChoiceBox<String> colorScheme; //Matthew Levine: 12/02/2013
    ...
    @FXML CheckBox completeTabs;
    @FXML CheckBox highlightText;
    ...
    @FXML protected void onApplyButtonClick(ActionEvent e) {
        ...
        //Matthew Levine added 12/01/2013
        //this is how DesktopController retrieves the current TextArea. Also,
        //this ridiculous toBack toFront business is necessary to update correctly
        //or else Java FX 2.2 threads don't sync properly with the focus command
        desktopController.getTabEditorContollers().get(
                desktopController.getTextTabPane().getSelectionModel(
                ).getSelectedItem()).getTextArea().toFront();
        tabPane.toBack();
        desktopController.textAreaEnhancer.forceUpdate(
                desktopController.getTabEditorContollers().get(
                        desktopController.getTextTabPane().getSelectionModel(
    ...
    private void saveFontTab() {
        ...
        //Matthew Levine 12/02/2013: Added ability to use colorscheme instead
        if ( !desktopController.colorScheme.equals(colorScheme.getValue()) ){
            String[] newColorScheme = DesktopController.colorSchemeMap.get(colorScheme.getValue());
            textFD.foreground = newColorScheme[0];
            textFD.background = newColorScheme[1];
            desktopController.colorScheme = colorScheme.getValue();
            assemblyForground.setValue(Color.web(newColorScheme[0]));
            assemblyBackground.setValue(Color.web(newColorScheme[1]));
        }
        else{
    ...
    private void saveOtherTab() {
        ...
        desktopController.getOtherSettings().completeTabs.set(this.completeTabs.isSelected());
        desktopController.getOtherSettings().highlightText.set(this.highlightText.isSelected());
    }
    ...
    private void setValues() {
        ...
        colorScheme.setValue(desktopController.colorScheme);
        ...
        completeTabs.selectedProperty().set(desktopController.getOtherSettings().completeTabs.get());
        highlightText.selectedProperty().set(desktopController.getOtherSettings().highlightText.get());
    }
